...
int zx_LEN_SO_a_EndpointReference(struct zx_ctx* c, struct zx_a_EndpointReference_s* x )
{
  ...
  for (se = x->AltAddress;
    se && se->g.tok == zx_tas3_AltAddress_ELEM;
    se = (struct zx_elem_s*)se->g.n)
    len += zx_LEN_SO_simple_elem(c,se, sizeof("tas3:AltAddress")-1, zx_ns_tab+(zx_tas3_NS >> ZX_TOK_NS_SHIFT));
...
char* zx_ENC_SO_a_EndpointReference(struct zx_ctx* c, struct zx_a_EndpointReference_s* x, char* p )
{
  ...
  for (se = x->AltAddress;
       se && se->g.tok == zx_tas3_AltAddress_ELEM;
       se = (struct zx_elem_s*)se->g.n)
    p = zx_ENC_SO_simple_elem(c, se, p, "tas3:AltAddress", sizeof("tas3:AltAddress")-1, zx_ns_tab+(zx_tas3_NS >> ZX_TOK_NS_SHIFT));
